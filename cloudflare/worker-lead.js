// Cloudflare Worker: Lead Handler
// Añade RESEND_KEY en Variables de Entorno
...
export default {
  async fetch(request) {
    ...
    const RESEND_KEY = typeof RESEND_KEY !== 'undefined' ? RESEND_KEY : '';
    if (!RESEND_KEY) return new Response(JSON.stringify({ error: 'RESEND_KEY no configurada' }), { status: 500, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
    ...
    try {
      ...
      if (email) {
        ...
        await fetch('https://api.resend.com/emails', {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${RESEND_KEY}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ from: 'OposicionesConIA <onboarding@resend.dev>', to: email, subject: subj + ' - OposicionesConIA', html: WRAP(content) })
        });
      }

      // Save to sheet
      try {
        await fetch('https://script.google.com/macros/s/AKfycbwAyZ33xDmHgVsOXDtMD2tksKwTI_h71vVzVOOiDfZjJ5g9UDsVEPIU-47R_KXxHGv6/exec', {
          method: 'POST', body: JSON.stringify({ date: new Date().toISOString(), name, email, phone, oposicion, ccaa, resource: r, source: data.source || 'web' })
        });
      } catch(e) {}
